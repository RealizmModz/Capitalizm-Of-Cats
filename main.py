...
def newUser():
	...
	if not db.find_one({"username": newUSERNAME}) == None:
		print('Oops! That username already exists! Please try again!')
		newUser()
	else:
		db.insert_one({
		...
		inv = db.find_one({"username": newUSERNAME})["inventory"]
		global catnip
		global cats
		global tiredCats
		global catFood
		...
		bell = inv["bell"]
		cloth = inv["cloth"]
		yarnBall = inv["yarnBall"]
		toyMouse = inv["toyMouse"]
		featherOnString = inv["featherOnString"]
		toyBird = inv["toyBird"]
...
def returningUser():
	...
	if db.count_documents({"username": USERNAME}) == 0:
		print('There are no matching accounts!')
		time.sleep(1)
		os.system('clear')
		newUser()
	else:
		if db.count_documents({"username": USERNAME, "password": PASSWORD}) == 1:
			print('Logged In!')
			inv = db.find_one({"username": USERNAME})["inventory"]
			global USER_NAME
			global catnip
			global cats
			global tiredCats
...
ZZZzz /,`.-'`'    -.  ;-;;,_
    ...
    """)
		...
		if choice == "1":
			...
			stat()
			print(
			 "How many cats would you like to hire? (15 Catnip per cat) or input 0 to buy max amount"
			)
			try:
				amount = int(input("amount: "))
				temp = amount * 15
				if temp <= catnip:
					db.find_one_and_update({  }, { '$set':{ "catnip" } })
					catnip -= temp
				if amount == 0:
					while catnip >= 15:
						catnip -= 15
